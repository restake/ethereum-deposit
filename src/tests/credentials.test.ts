...
describe("Credential", () => {
    ...
    it("credential does not allow invalid amount (< MIN_DEPOSIT_AMOUNT)", () => {
        ...
        expect(invalidCredential.amount).to.be.lt(MIN_DEPOSIT_AMOUNT)
            && expect(() => invalidCredential.depositData).to.throw("Invalid deposit amount");
    });
    ...
    it("credential does not allow invalid amount (> MAX_DEPOSIT_AMOUNT)", () => {
        ...
        expect(invalidCredential.amount).to.be.gt(MAX_DEPOSIT_AMOUNT)
            && expect(() => invalidCredential.depositData).to.throw("Invalid deposit amount");
    });
    ...
    it("credential returns correct signing public key hex", () => {
        ...
        expect(signingPublicKey)
            .to.equal("0xa84128adbc8a1b1be6d06d8c24b9371f9cd71709cf287df628d8f1c89eb6be94e15c79453e290dcede1434a8f623b8e4");
    });

    it("credential returns correct withdrawal public key hex", () => {
        const withdrawalPublicKey = credential.withdrawalPublicKey.toHex();
        // TODO: verify!
        expect(withdrawalPublicKey)
            .to.equal("0x892f3b487595f337437fd08fba9f32b51c4e62e3fc3d154ca43bcf9852183825e0e36e2c3578d0d01d9d3602e44abd9c");
    });
});
